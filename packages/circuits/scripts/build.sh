...
function dev_trusted_setup() {
    ...
    node $SNARKJS_PATH groth16 setup "$BUILD_DIR"/digilocker-verifier.r1cs "$PTAU_PATH" "$PARTIAL_ZKEYS_DIR"/circuit_0000.zkey
...
function generate_witness() {
    echo "Gen witness..."
    npx ts-node ./scripts/generate-input.ts
...
case "$1" in
    ...
    ;;
    *)
        echo "Usage: $0 {build|setup|gen-proof|gen-contract|gen-witness|verify-proof}"
    ;;
esac
